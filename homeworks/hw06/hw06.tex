...
\easysubproblem{Prove a Jensens-like inequality for all convex functions $h$ for all discrete r.v.'s $X$ with finite support.}\spc{8}
...
\begin{enumerate}
\easysubproblem{Given $\Xoneton \sim$ with finite $\mu$ and $\sigsq$, show that $\Xbar_n \convp \mu$, the \qu{weak} weak law of large numbers.}\spc{3}
...
\hardsubproblem{Given $\Xoneton \sim$ with finite $\mu$ but \emph{not finite} $\sigsq$, show that $\Xbar_n \convd \mu$. Hint: use the characteristic functions and a similar argument we used to prove the central limit theorem.}\spc{15}

\easysubproblem{Use (b) and (c) to show that given $\Xoneton \sim$ with finite $\mu$ but \emph{not finite} $\sigsq$, show that $\Xbar_n \convp \mu$, the \qu{strong} weak law of large numbers.}\spc{1}
